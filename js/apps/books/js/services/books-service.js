...
function saveBook(bookId, review) {
  review.id = utilService.makeId();
  return getBookById(bookId).then((book) => {
    if (!book.reviews || !book.reviews.length) book.reviews = [review];
...
function getGoogleResults(searchParam) {
  const GOOGLE_API = `https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20${searchParam}`;
  return axios.get(GOOGLE_API).then((res) => res.data);
}
...
function getNextPrevBook(bookId) {
  return query().then((books) => {
    ...
    const nextPrev = {
      nextBookId: books[idx + 1] ? books[idx + 1].id : books[0].id,
      prevBookId: books[idx - 1] ? books[idx - 1].id : books[books.length - 1].id,
    };
    return nextPrev;
  });
}
